...
const ScreenManager = (() => {
  ...
  function calculateScreenTimes(prayer) {
    ...
    // Untuk Imsak dan Syuruq (tidak ada countdown)
  ...
  /* ===============================
     ...
     function openMainForm() {
      ...
      const html = `
        <div class="popup-overlay" id="screenMainForm" style="display:flex; z-index:10050">
          <div class="popup-content position-relative" style="max-width:500px; max-height:80vh; overflow-y:auto">
            ...
            <div class="row g-2 mb-4">
              ${Object.keys(PRAYER_NAMES).map(prayer => `
                <div class="col-6 col-md-4">
                  <button class="btn btn-outline-primary w-100 prayer-btn" 
            ...
            <div class="d-grid gap-2">
              <button class="btn btn-success" onclick="ScreenManager.forceOn()">
                <i class="bi bi-power"></i> Hidupkan Screen Sekarang
              </button>
              <button class="btn btn-danger" onclick="ScreenManager.forceOff()">
                <i class="bi bi-power"></i> Matikan Screen Sekarang
              </button>
              <button class="btn btn-secondary" onclick="ScreenManager.resetAllSettings()">
                <i class="bi bi-arrow-clockwise"></i> Reset Semua Pengaturan
              </button>
            </div>
          </div>
        </div>
    ...
    function openPrayerForm(prayer) {
      ...
      const settings = screenSettings[prayer];
      const isImsakSyuruq = (prayer === 'imsak' || prayer === 'syuruq');
          ...
          <div class="popup-content position-relative" style="max-width:500px; max-height:80vh; overflow-y:auto">
            ...
            <div class="mb-4">
              <label class="form-label fw-bold mb-2">
                ${isImsakSyuruq ? 'Screen OFF (menit sebelum waktu):' : 'Screen ON (menit sebelum Countdown Adzan):'}
              </label>
              <div class="btn-group btn-group-toggle flex-wrap" style="gap:5px">
              ...
              <small class="text-muted">
                ${isImsakSyuruq 
                  ? `Screen akan HIDUP ${settings.screenOn} menit sebelum ${PRAYER_NAMES[prayer]}`
                  : `Screen akan HIDUP ${settings.screenOn} menit sebelum Countdown Adzan ${PRAYER_NAMES[prayer]}`}
              </small>
            </div>
            
            <div class="mb-4">
              <label class="form-label fw-bold mb-2">
                ${isImsakSyuruq ? 'Screen ON (menit setelah waktu):' : 'Screen OFF (menit setelah Overlay Black):'}
              </label>
              <div class="btn-group btn-group-toggle flex-wrap" style="gap:5px">
                ${DURATION_BUTTONS.map(min => `
            ...
            <div class="alert alert-info">
              ...
              ${isImsakSyuruq 
                ? `• Screen ON: ${settings.screenOn} menit sebelum ${PRAYER_NAMES[prayer]}<br>• Screen OFF: ${settings.screenOff} menit setelah ${PRAYER_NAMES[prayer]}`
                : `• Screen ON: ${settings.screenOn} menit sebelum Countdown Adzan<br>• Screen OFF: ${settings.screenOff} menit setelah Overlay Black`}
            </div>
            
            <div class="d-grid gap-2">
              <button class="btn btn-success" onclick="ScreenManager.savePrayerSettings()">
                <i class="bi bi-check-lg"></i> Simpan Pengaturan
              </button>
              <button class="btn btn-secondary" onclick="ScreenManager.openMainForm()">
  ...
  function updatePrayerFormUI(prayer) {
    ...
    if (infoDiv && currentPrayer) {
      ...
      const isImsakSyuruq = (currentPrayer === 'imsak' || currentPrayer === 'syuruq');
